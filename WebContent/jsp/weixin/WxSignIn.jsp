				...
				if(status == true) {
					...
					$.post("resource?WxSignIn",{actionType:"getLocation","Latitude":lat,"Longitude":lng}, function(data){
						if(data.error) {
							...
							var html = "";
							html = "请选择您的位置: <input type='button' value='签到' onclick='javascript:signclick()'/><br/>\n";
							html = html +	"<form id='sign' name='sign' method='POST' action='resource?WxSignIn'>\n";
							...
							<%if (session.getAttribute("errorMsg") != null && session.getAttribute("errorMsg").toString().contains("微信绑定")) {%>
							$("#tip").hide();
							$("#bmap").hide();
							<%}%>
						}
...
<body>
	<form action="resource?WxSignIn" name="form1" id="form1" method="post"
		...
		<div>
			<ul data-role="listview">
				...
				<li>
					<div>
						<span style="float: right;"><%=pageBean.inputTime(i, "ATD_IN_TIME").substring(5, 16)%><span
							style="margin-left: 15px">（<%=i + 1%>）
						</span></span> <span style="color: #FF8C00"><%=pageBean.labelValue(i, "USER_ID_NAME")%></span>
					</div>
					<div><%=pageBean.inputTime(i, "ATD_IN_PLACE")%></div>
